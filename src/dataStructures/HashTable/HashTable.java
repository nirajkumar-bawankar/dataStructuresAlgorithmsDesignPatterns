...
public class HashTable {
    ...
    /**
     * A function that maps keys to slots in a way that makes each slot in the
     * hash table have equal probability of being filled for the actual set of
     * keys being used.
     */
     ...
     */
    public void midSquareHashFunction() {
     ...
     *
     */
    public int summationStringHashFunction(String x, int M) {
	char ch[];
	ch = x.toCharArray();
     ...
     * the range 0 to (M - 1) using the modulus operator.
     *
     ...
     *
     * Note: for any sufficiently long string, the sum for the integer quantities
     * will typically cause a 32-bit integer to overflow (thus losing some of
     * the high-order bits) because the resulting values are so large. But
     * this causes no problems when the goal is to compute a hash function.
     */
    public long stringFoldingHashFunction(String s, int M) {
